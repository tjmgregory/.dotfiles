...
# https://gist.github.com/rorydbain/e20e6ab0c7cc027fc1599bd2e430117d
# 
# ASSUMPTIONS & SETUP:
...
w() {
    ...
    if [[ -z "$wt_path" || ! -d "$wt_path" ]]; then
        ...
        # Check if a branch with the exact worktree name exists on any remote
        local exact_branch_exists=false
        local remote_branch=""
        ...
        (cd "$projects_dir/$project" && {
            ...
            # Check if branch exists on any remote with exact name
            for remote in $(git remote); do
                if git ls-remote --heads "$remote" "refs/heads/$worktree" | grep -q .; then
                    exact_branch_exists=true
                    remote_branch="$remote/$worktree"
                    break
                fi
            done
        })
        
        if [[ "$exact_branch_exists" == "true" ]]; then
            # Exact branch name exists on remote, check it out
            echo "Found existing remote branch: $remote_branch"
            (cd "$projects_dir/$project" && git worktree add "$wt_path" "$worktree") || {
                echo "Failed to create worktree from existing branch"
                return 1
            }
        else
            # No exact match found, create new branch with username prefix
